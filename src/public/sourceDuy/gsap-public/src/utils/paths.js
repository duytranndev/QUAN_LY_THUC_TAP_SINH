...
let _svgPathExp = /[achlmqstvz]|(-?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi,
    ...
    _wrapProgress = (progress) =>
        Math.round(((progress + _largeNum) % 1) * _roundingNum) /
            _roundingNum || (progress < 0 ? 0 : 1), //if progress lands on 1, the % will make it 0 which is why we || 1, but not if it's negative because it makes more sense for motion to end at 0 in that case.
    ...
    for (j = startIndex + 2; j < endIndex; j += 6) {
        ...
        if (_abs(x4) < 1e-5 && _abs(y4) < 1e-5 && _abs(x2) + _abs(y2) < 1e-5) {
            ...
            for (i = 1; i <= resolution; i++) {
                ...
                xd =
                    xd1 -
                    (xd1 = (t * t * x4 + 3 * inv * (t * x3 + inv * x2)) * t);
                yd =
                    yd1 -
                    (yd1 = (t * t * y4 + 3 * inv * (t * y3 + inv * y2)) * t);
                l = _sqrt(yd * yd + xd * xd);
                if (l < min) {
                    min = l;
                }
