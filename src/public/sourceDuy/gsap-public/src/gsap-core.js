...
let _config = {
    ...
    _animationCycle = (tTime, cycleDuration) =>
        (tTime /= cycleDuration) && ~~tTime === tTime ? ~~tTime - 1 : ~~tTime,
        ...
        let prevRatio = tween.ratio,
            ratio =
                ...
                ((tween._ts < 0 || tween._dp._ts < 0) &&
                    ...
                    tween.data !== 'isStart')
                    ? 0
                    : 1, // check parent's _lock because when a timeline repeats/yoyos and does its artificial wrapping, we shouldn't force the ratio back to 0. Also, if the tween or its parent is reversed and the totalTime is 0, we should go to a ratio of 0.
        ...
        return (i, target, a) => {
            ...
            if (!distances) {
                ...
                if (!wrapAt) {
                    ...
                    while (
                        max <
                            (max = a[wrapAt++].getBoundingClientRect().left) &&
                        wrapAt < l
                    ) {}
                    wrapAt--;
                }
...
_forEachName('Linear,Quad,Cubic,Quart,Quint,Strong', (name, i) => {
    ...
    _insertEase(
        ...
        (p) => 1 - (1 - p) ** power,
        (p) =>
            p < 0.5 ? (p * 2) ** power / 2 : 1 - ((1 - p) * 2) ** power / 2,
    );
});
...
const _gsap = {
    ...
    quickSetter(target, property, unit) {
        ...
        if (target.length > 1) {
            let setters = target.map((t) =>
                    gsap.quickSetter(t, property, unit),
                ),
                l = setters.length;
            return (value) => {
                let i = l;
        ...
        return Plugin
            ? setter
            : (value) =>
                  setter(target, p, unit ? value + unit : value, cache, 1);
    },
    isTweening(targets) {
