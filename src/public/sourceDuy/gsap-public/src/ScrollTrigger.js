...
let gsap,
    ...
    _getSizeFunc = (scroller, isViewport, { d, d2, a }) =>
        (a = _getProxyProp(scroller, 'getBoundingClientRect'))
            ? () => a()[d]
            : () =>
                  (isViewport ? _win['inner' + d2] : scroller['client' + d2]) ||
                  0,
    ...
    _createMarker = (
        ...
        css +=
            'position:' +
            (isScroller && useFixedPosition ? 'fixed;' : 'absolute;');
        ...
        for (_i = 0; _i < _triggers.length; _i++) {
            ...
            if (!media || trigger.media === media) {
                if (kill) {
                    ...
                    trigger.scroll.rec ||
                        (trigger.scroll.rec = trigger.scroll()); // record the scroll positions so that in each refresh() we can ensure that it doesn't shift. Remember, pinning can make things change around, especially if the same element is pinned multiple times. If one was already recorded, don't re-record because unpinning may have occurred and made it shorter.
                    trigger.revert();
                }
            }
        }
    ...
    _swapPinIn = (pin, spacer, cs, spacerState) => {
        if (pin.parentNode !== spacer) {
            ...
            spacerStyle[_padding] = pinStyle[_margin] = pinStyle[
                _top
            ] = pinStyle[_left] = '0';
    ...
    init(vars, animation) {
        ...
        let direction = vars.horizontal ? _horizontal : _vertical,
            ...
            useFixedPosition =
                'pinType' in vars
                    ? vars.pinType === 'fixed'
                    : isViewport ||
                      _getProxyProp(scroller, 'pinType') === 'fixed',
            callbacks = [
                vars.onEnter,
                vars.onLeave,
                vars.onEnterBack,
            ...
            self = this,
            onRefreshInit =
                vars.onRefreshInit && (() => vars.onRefreshInit(self)),
        ...
        self.refresh = (soft) => {
            ...
            let size = getScrollerSize(),
                ...
                parsedStart =
                    vars.start ||
                    (vars.start === 0 ? 0 : pin || !trigger ? '0 0' : '0 100%'),
                triggerIndex =
            ...
            if (pin) {
                ...
                if (useFixedPosition) {
                    ...
                    override[_margin] = override[_margin + _Top] = override[
                        _margin + _Right
                    ] = override[_margin + _Bottom] = override[
                        _margin + _Left
                    ] = '0';
            ...
            prevAnimProgress &&
                isToggle &&
                animation.progress(prevAnimProgress, true);
        ...
        self.update = (reset, recordVelocity) => {
            ...
            if (clipped !== prevProgress && self.enabled) {
                ...
                if (pin) {
                    ...
                    if (!useFixedPosition) {
                        ...
                        if (pinReparent) {
                            if (!reset && (isActive || action)) {
                                ...
                                _reparent(
                                    ...
                                    _body,
                                    bounds.top +
                                        (direction === _vertical ? offset : 0) +
                                        _px,
                                    bounds.left +
                                        (direction === _vertical ? 0 : offset) +
                                        _px,
                                );
                            } else {
                        ...
                        _setState(
                            isActive || action ? pinActiveState : pinState,
                        );
                        (pinChange !== change && clipped < 1 && isActive) ||
                            pinSetter(
                                pinStart +
                                    (clipped === 1 && !action ? pinChange : 0),
                            );
                    }
                }
                snap &&
            ...
            if (markerEndSetter) {
                markerStartSetter(
                    scroll + (markerStartTrigger._isFlipped ? 1 : 0),
                );
