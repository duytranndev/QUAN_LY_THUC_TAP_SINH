...
let _win,
    ...
    _convertToUnit = (target, property, value, unit) => {
        let curValue = parseFloat(value) || 0,
            curUnit =
                (value + '').trim().substr((curValue + '').length) || 'px', // some browsers leave extra whitespace at the beginning of CSS variables, hence the need to trim()
            ...
            while ((result = _numWithUnitExp.exec(end))) {
                ...
                if (
                    ...
                    if (startUnit !== endUnit) {
                        startNum =
                            _convertToUnit(target, prop, startValue, endUnit) ||
                            0;
                    }
...
_forEachName('padding,margin,Width,Radius', (name, index) => {
    let t = 'Top',
        ...
        props = (index < 3
            ? [t, r, b, l]
            : [t + l, t + r, b + r, b + l]
        ).map((side) => (index < 2 ? name + side : 'border' + side + name));
