...
let gsap,
    ...
    _renderQueueTimeout = () =>
        !_renderQueue.length && gsap.ticker.remove(_renderQueueTick),
    ...
    _addListener = (element, type, func, capture) => {
        if (element.addEventListener) {
            ...
            touchType &&
                type !== touchType &&
                element.addEventListener(type, func, capture); //some browsers actually support both, so must we. But pointer events cover all.
        }
    },
    ...
    _getBounds = (target, context) => {
        ...
        if (!e.nodeType && e !== _win) {
            ...
            return {
                ...
                top: top,
                width:
                    (target.max || target.maxX || target.maxRotation || 0) -
                    left,
    ...
    _initCore = (required) => {
        if (_windowExists() && document.body) {
            ...
            _isAndroid =
                nav && nav.userAgent.toLowerCase().indexOf('android') !== -1; //Android handles touch events in an odd way and it's virtually impossible to "feature test" so we resort to UA sniffing
            _isTouchDevice =
            ...
            _touchEventLookup = (function (types) {
                ...
                let standard = types.split(','),
                    converted = ('onpointerdown' in _tempDiv
...
class EventDispatcher {
    ...
    dispatchEvent(type) {
        ...
        (this._listeners[type] || []).forEach(
            (callback) =>
                callback.call(this, { type: type, target: this.target }) ===
                    false && (result = false),
        );
...
export class Draggable extends EventDispatcher {
    constructor(target, vars) {
        ...
        this.edgeResistance = isNaN(vars.edgeResistance)
            ? 1
        ...
        let type = (vars.type || 'x,y').toLowerCase(),
            ...
            getPropAsNum = (property, unit) =>
                parseFloat(gsCache.get(target, property, unit)),
                    ...
                    while (e && !isRoot) {
                        ...
                        if (allowY) {
                            ...
                            if (gap < 0) {
                                ...
                                pointerY >
                                    rect.bottom - autoScrollMarginBottom &&
                                gap
                            ) {
            ...
            syncXY = (skipOnUpdate, skipSnap) => {
                ...
                if (xyMode) {
                    ...
                    self.x =
                        parseInt(target.style.left || (cs || {}).left, 10) || 0;
                }
                if (
                    (snapX || snapY || snapXY) &&
                    !skipSnap &&
            ...
            buildSnapFunc = (snap, min, max, factor) => {
                ...
                if (_isFunction(snap)) {
                    return (n) => {
                        let edgeTolerance = !self.isPressed
                            ? 1
                            : 1 - self.edgeResistance; //if we're tweening, disable the edgeTolerance because it's already factored into the tweening values (we don't want to apply it multiple times)
                        return (
                            snap.call(
                                self,
                                n > max
                        ...
                        if (snap.points) {
                            ...
                            if (allowY) {
                                snapY = buildSnapFunc(
                                    snapIsRaw
                                        ? snap
                                        : snap.y || snap.top || snap.scrollTop,
                                    minY,
                                    maxY,
                                    scrollProxy ? -1 : 1,
                                );
            ...
            animate = (inertia, forceZeroVelocity) => {
                ...
                if (inertia && InertiaPlugin) {
                    if (inertia === true) {
                        ...
                        inertia = {
                            resistance:
                                (vars.throwResistance ||
                                    vars.resistance ||
                                    1000) / (rotationMode ? 10 : 1),
                        };
                        if (rotationMode) {
                            ...
                            if (
                                snap.points ||
                                (_isArray(snap) && _isObject(snap[0]))
                            ) {
            ...
            updateMatrix = (shiftStart) => {
                ...
                if (
                    ...
                    p = start
                        .inverse()
                        .apply({ x: startPointerX, y: startPointerY });
                    matrix.apply(p, p);
                    startPointerX = p.x;
                    startPointerY = p.y;
                }
            ...
            recordStartPositions = () => {
                ...
                if (hasBounds && edgeTolerance) {
                    if (startElementX > maxX) {
                        startElementX =
                            maxX + (startElementX - maxX) / edgeTolerance;
                    } else if (startElementX < minX) {
                        startElementX =
                            minX - (minX - startElementX) / edgeTolerance;
                    }
                    if (!rotationMode) {
                        if (startElementY > maxY) {
                            startElementY =
                                maxY + (startElementY - maxY) / edgeTolerance;
                        } else if (startElementY < minY) {
                            startElementY =
                                minY - (minY - startElementY) / edgeTolerance;
                        }
                    }
                }
            ...
            onPress = (e, force) => {
                ...
                if (_touchEventLookup[e.type]) {
                    ...
                    touchEventTarget = ~e.type.indexOf('touch')
                        ...
                        : ownerDoc; //pointer-based touches (for Microsoft browsers) don't remain locked to the original target like other browsers, so we must use the document instead. The event type would be "MSPointerDown" or "pointerdown".
                ...
                if (
                    ...
                    _placeholderDiv.style.width =
                        target.parentNode.scrollWidth + 'px';
                ...
                if (
                    !rotationMode &&
                    (vars.cursor !== false || vars.activeCursor)
                ) {
                    i = triggers.length;
                    while (--i > -1) {
            ...
            onMove = (e) => {
                ...
                if (
                    ...
                    if (
                        (dx !== dy &&
                            (dx > minimumMovement || dy > minimumMovement)) ||
                        (_isAndroid &&
                        ...
                        if (
                            ...
                            _addListener(
                                ...
                                'touchforcechange',
                                _preventDefault,
                            ); // prevents native touch scrolling from taking over if the user started dragging in the other direction in iOS Safari
                        }
                        if (
            ...
            setPointerPosition = (pointerX, pointerY, invokeOnMove) => {
                ...
                if (rotationMode) {
                    ...
                    if (
                        yChange < minimumMovement &&
                        yChange > -minimumMovement
                    ) {
                        yChange = 0;
                    }
                    if (
                        xChange < minimumMovement &&
                        xChange > -minimumMovement
                    ) {
                        xChange = 0;
                    }
                    if (
                        (self.lockAxis || self.lockedAxis) &&
                        (xChange || yChange)
                    ) {
            ...
            onRelease = (e, force) => {
                if (
                    ...
                    (e &&
                        touchID != null &&
                        !force &&
                        ((e.pointerId && e.pointerId !== touchID) ||
                            (e.changedTouches &&
                                !_hasTouchID(e.changedTouches, touchID))))
                ) {
                    //for some Microsoft browsers, we must attach the listener to the doc rather than the trigger so that when the finger moves outside the bounds of the trigger, things still work. So if the event we're receiving has a pointerId that doesn't match the touchID, ignore it (for multi-touch)
                ...
                let originalEvent = e,
                    ...
                    isContextMenuRelease =
                        ...
                        e &&
                        (e.ctrlKey || e.which > 2),
                    placeholderDelayedCall = gsap.delayedCall(
                        0.001,
                        removePlaceholder,
                    ),
                    touches,
                ...
                if (!rotationMode) {
                    ...
                    while (--i > -1) {
                        _setStyle(
                            ...
                            'cursor',
                            vars.cursor ||
                                (vars.cursor !== false ? _defaultCursor : null),
                        );
                    }
                }
                if (wasDragging) {
            ...
            updateScroll = (e) => {
                if (e && self.isDragging && !scrollProxy) {
                    ...
                    if (deltaX || deltaY) {
                        if (matrix) {
                            startPointerX -=
                                deltaX * matrix.a + deltaY * matrix.c;
                            startPointerY -=
                                deltaY * matrix.d + deltaX * matrix.b;
                        } else {
            ...
            onClick = (e) => {
                ...
                let time = _getTime(),
                    ...
                    alreadyDispatched =
                        recentlyClicked && clickDispatch === clickTime,
                    defaultPrevented =
                    ...
                    trusted =
                        ...
                        (e.isTrusted == null &&
                            recentlyClicked &&
                            alreadyDispatched); //note: Safari doesn't support isTrusted, and it won't properly execute native behavior (like toggling checkboxes) on the first synthetic "click" event - we must wait for the 2nd and treat it as trusted (but stop propagation at that point). Confusing, I know. Don't you love cross-browser compatibility challenges?
                if (
                    (alreadyDispatched ||
                        (recentlyDragged &&
                            self.vars.suppressClickOnDrag !== false)) &&
                ...
                if (
                    ...
                    (!alreadyDispatched ||
                        (trusted && !alreadyDispatchedTrusted))
                ) {
        ...
        this.timeSinceDrag = () =>
            self.isDragging ? 0 : (_getTime() - dragEndTime) / 1000;
        this.timeSinceClick = () => (_getTime() - clickTime) / 1000;
        this.hitTest = (target, threshold) =>
        ...
        if (~type.indexOf('scroll')) {
            ...
            target.style.overflowY =
                allowY && !_isTouchDevice ? 'auto' : 'hidden';
            target.style.overflowX =
                allowX && !_isTouchDevice ? 'auto' : 'hidden';
