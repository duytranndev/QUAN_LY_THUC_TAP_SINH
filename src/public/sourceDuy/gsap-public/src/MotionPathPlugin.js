...
let _xProps = ['x', 'translateX', 'left', 'marginLeft'],
    ...
    _originToPoint = (element, origin, parentMatrix) => {
        ...
        return parentMatrix.apply(
            x || y ? m.apply({ x: x, y: y }) : { x: m.e, y: m.f },
        );
    },
    ...
    _align = (
        rawPath,
        target,
        { align, matrix, offsetX, offsetY, alignOrigin },
    ) => {
        let x = rawPath[0][0],
            y = rawPath[0][1],
...
export const MotionPathPlugin = {
    ...
    convertToPath: (targets, swap) =>
        _toArray(targets).map((target) =>
            convertToPath(target, swap !== false),
        ),
    convertCoordinates(fromElement, toElement, point) {
        let m = getGlobalMatrix(toElement, true, true).multiply(
