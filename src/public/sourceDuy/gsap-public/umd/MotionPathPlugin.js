(function (global, factory) {
    ...
    function sliceRawPath(rawPath, start, end) {
        ...
        var path = copyRawPath(
            ...
            invertedOrder =
                (sameSegment && si > ei) || (sameBezier && s.t > e.t),
            sShift,
            eShift,
            i,
            copy,
        ...
        if (wrap || loops) {
            ...
            if (start !== end) {
                sShift = subdivideSegment(
                    sSeg,
                    si,
                    sameBezier ? s.t / e.t : s.t,
                );
    ...
    var _xProps = ['x', 'translateX', 'left', 'marginLeft'],
        ...
        _segmentToRawPath = function _segmentToRawPath(
            ...
            if (vars.type === 'cubic') {
                ...
                segment.unshift(
                    _getPropNum(target, x, vars.unitX),
                    y ? _getPropNum(target, y, vars.unitY) : 0,
                );
        ...
        _getAlignMatrix = function _getAlignMatrix(
            ...
            var parentMatrix = getGlobalMatrix(
                    fromElement.parentNode,
                    true,
                    true,
                ),
                m = parentMatrix.clone().multiply(getGlobalMatrix(toElement)),
                fromPoint = _originToPoint(
                    fromElement,
                    fromOrigin,
                    parentMatrix,
                ),
        ...
        _sliceModifier = function _sliceModifier(start, end) {
            return function (rawPath) {
                return start || end !== 1
                    ? sliceRawPath(rawPath, start, end)
                    : rawPath;
            };
        };
    ...
    var MotionPathPlugin = {
        ...
        init: function init(target, vars) {
            ...
            var rawPaths = [],
                ...
                slicer = _sliceModifier(
                    vars.start,
                    'end' in vars ? vars.end : 1,
                ),
                rawPath,
                p,
                x,
        ...
        render: function render(ratio, data) {
            ...
            data.rotate &&
                data.rSet(
                    data.target,
                    data.rProp,
                    rawPaths[0].angle * (data.radians ? _DEG2RAD$1 : 1) +
                        data.rOffset +
                        data.ru,
                    data,
                    ratio,
